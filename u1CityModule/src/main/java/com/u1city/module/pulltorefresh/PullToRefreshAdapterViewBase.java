...
import com.u1city.module.pulltorefresh.internal.IndicatorLayout;
...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
	    ...
	    if(scrllPosition != null)
	    {
	        scrllPosition.getState(view, firstVisibleItem, visibleItemCount, totalItemCount);
	    }
	...
	public final void onScrollStateChanged(final AbsListView view, final int state) {
		 ...
		 */
	    if(scrllPosition != null){
	        scrllPosition.getIdel(state);
	    }
	    Debug.d(LOG_TAG, "==============state===================: " + state);
    ...
    //添加方法控制片判断adapter子item数目是否为0
    private boolean enableGetVisibleWithCount = true;

    /**
     * 添加方法控制片判断adapter子item数目是否为0
     * @param enableGetVisibleWithCount 是否使用adapter.getCount()来判断
     * */
    public void setEnableGetVisibleWithCount(boolean enableGetVisibleWithCount) {
        this.enableGetVisibleWithCount = enableGetVisibleWithCount;
    }

    private boolean isFirstItemVisible() {
		final Adapter adapter = mRefreshableView.getAdapter();

        Debug.d("tag", "" + adapter.getCount());

		if (null == adapter || (enableGetVisibleWithCount && adapter.getCount() <= 0) || (!enableGetVisibleWithCount && adapter.isEmpty())) {
			if (DEBUG) {
			...
			if (mRefreshableView.getFirstVisiblePosition() <= 1) {
				...
				if (firstVisibleChild != null) {
					Debug.d("tag", "firstVisibleChild:" + firstVisibleChild.getTop() + " -- mRefreshableView:" + mRefreshableView.getTop());
	...
	private ScrllPosition scrllPosition;
	
	public interface ScrllPosition{
	    public void getState(AbsListView view, int firstVisibleItem, int visibleItemCount,int totalItemCount);
	    public void getIdel(int state);
	}
	
	public void setOnCustomScrllLisenter(ScrllPosition scrllPosition){
	    this.scrllPosition = scrllPosition;
	}
}
